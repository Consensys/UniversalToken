...
const issueOnMultiplePartitions = async (
  ...
  for (var i = 0; i < _partitions.length; i++) {
    await _contract.issueByPartition(_partitions[i], _recipient, _amounts[i], ZERO_BYTES32, { from: _owner });
  }
};
...
contract('ERC1400', function ([owner, operator, controller, controller_alternative1, controller_alternative2, tokenHolder, recipient, unknown]) {
    ...
    describe('contract creation', function () {
        it('fails deploying the contract if granularity is lower than 1', async function () {
            await shouldFail.reverting(ERC1400.new('ERC1400Token', 'DAU', 0, [controller], partitions));
        });
    });
    ...
    describe('canImplementInterfaceForAddress', function () {
        beforeEach(async function () {
            this.token = await ERC1400.new('ERC1400Token', 'DAU', 1, [controller], partitions);
          });
        describe('when interface hash is correct', function () {
            it('returns ERC1820_ACCEPT_MAGIC', async function () {
            const canImplement1400 = await this.token.canImplementInterfaceForAddress(soliditySha3(ERC1400_INTERFACE_NAME), ZERO_ADDRESS);          
            assert.equal(soliditySha3(ERC1820_ACCEPT_MAGIC), canImplement1400);
            const canImplement20 = await this.token.canImplementInterfaceForAddress(soliditySha3(ERC20_INTERFACE_NAME), ZERO_ADDRESS);          
  ...
  describe('transfer', function () {
    beforeEach(async function () {
      this.token = await ERC1400.new('ERC1400Token', 'DAU', 1, [controller], partitions);
      await this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner });
    });
    ...
    describe('when the amount is a multiple of the granularity', function () {
      ...
      describe('when the amount is not a multiple of the granularity', function () {
        it('reverts', async function () {
          this.token = await ERC1400.new('ERC1400Token', 'DAU', 2, [], partitions);
          await this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner });
            ...
            describe('when the sender has enough balance for those default partitions', function () {
                describe('when the sender has defined custom default partitions', function () {
                  it('transfers the requested amount', async function () {
                    ...
                    await this.token.transferWithData(recipient, 2.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder });
                  ...
                  it('emits a sent event', async function () {
                    await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
                    const { logs } = await this.token.transferWithData(recipient, 2.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder });
        
                    assert.equal(logs.length, 2 * partitions.length);
        
                    assertTransferEvent([logs[0], logs[1]], partition3, tokenHolder, tokenHolder, recipient, issuanceAmount, ZERO_BYTES32, null);
                    assertTransferEvent([logs[2], logs[3]], partition1, tokenHolder, tokenHolder, recipient, issuanceAmount, ZERO_BYTES32, null);
                    assertTransferEvent([logs[4], logs[5]], partition2, tokenHolder, tokenHolder, recipient, 0.5 * issuanceAmount, ZERO_BYTES32, null);
                  });
                });
              ...
              describe('when the sender does not have enough balance for those default partitions', function () {
                it('reverts', async function () {
                  await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
                  await shouldFail.reverting(this.token.transferWithData(recipient, 3.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder }));
                });
              });
        });
        describe('when the recipient is the zero address', function () {
            it('reverts', async function () {
                ...
                await shouldFail.reverting(this.token.transferWithData(ZERO_ADDRESS, 2.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder }));
            });
        });
      });
      describe('when the amount is not a multiple of the granularity', function () {
        it('reverts', async function () {
            this.token = await ERC1400.new('ERC1400Token', 'DAU', 2, [controller], partitions);
            await issueOnMultiplePartitions(this.token, owner, tokenHolder, partitions, [issuanceAmount, issuanceAmount, issuanceAmount]);
            await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
            await assertBalances(this.token, tokenHolder, partitions, [issuanceAmount, issuanceAmount, issuanceAmount]);

            await shouldFail.reverting(this.token.transferWithData(recipient, 3, ZERO_BYTES32, { from: tokenHolder }));
          });
      });
    });
    describe('when defaultPartitions have not been defined', function () {
      it('reverts', async function () {
        this.token = await ERC1400.new('ERC1400Token', 'DAU', 1, [controller], []);
        await issueOnMultiplePartitions(this.token, owner, tokenHolder, partitions, [issuanceAmount, issuanceAmount, issuanceAmount]);
        await shouldFail.reverting(this.token.transferWithData(recipient, 2.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder }));
      });
    });
  });
            ...
            describe('when defaultPartitions have been defined', function () {
                describe('when the sender has enough balance for those default partitions', function () {
                  it('transfers the requested amount', async function () {
                    ...
                    await this.token.transferFromWithData(tokenHolder, recipient, 2.5 * issuanceAmount, ZERO_BYTES32, { from: operator });
                  ...
                  it('emits a sent event', async function () {
                    await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
                    const { logs } = await this.token.transferFromWithData(tokenHolder, recipient, 2.5 * issuanceAmount, ZERO_BYTES32, { from: operator });
        
                    assert.equal(logs.length, 2 * partitions.length);
        
                    assertTransferEvent([logs[0], logs[1]], partition3, operator, tokenHolder, recipient, issuanceAmount, ZERO_BYTES32, null);
                    assertTransferEvent([logs[2], logs[3]], partition1, operator, tokenHolder, recipient, issuanceAmount, ZERO_BYTES32, null);
                    assertTransferEvent([logs[4], logs[5]], partition2, operator, tokenHolder, recipient, 0.5 * issuanceAmount, ZERO_BYTES32, null);
                  });
                });
                describe('when the sender does not have enough balance for those default partitions', function () {
                  it('reverts', async function () {
                    await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
                    await shouldFail.reverting(this.token.transferFromWithData(tokenHolder, recipient, 3.5 * issuanceAmount, ZERO_BYTES32, { from: operator }));
                  });
                });
              });
              describe('when defaultPartitions have not been defined', function () {
                it('reverts', async function () {
                  ...
                  await shouldFail.reverting(this.token.transferFromWithData(tokenHolder, recipient, 2.5 * issuanceAmount, ZERO_BYTES32, { from: operator }));
                });
              });
        });
        describe('when the recipient is the zero address', function () {
            it('reverts', async function () {
                ...
                await shouldFail.reverting(this.token.transferFromWithData(tokenHolder, ZERO_ADDRESS, 2.5 * issuanceAmount, ZERO_BYTES32, { from: operator }));
              });
        });
      });
      describe('when the amount is not a multiple of the granularity', function () {
        it('reverts', async function () {
            ...
            await shouldFail.reverting(this.token.transferFromWithData(tokenHolder, recipient, 3, ZERO_BYTES32, { from: operator }));
          });
      });
    });
    describe('when the operator is not approved', function () {
    ...
    describe('when the sender has enough balance for this partition', function () {
        describe('when the transfer amount is not equal to 0', function () {
          it('transfers the requested amount', async function () {
            ...
            await this.token.transferByPartition(partition1, recipient, transferAmount, ZERO_BYTES32, { from: tokenHolder });
            await this.token.transferByPartition(partition1, recipient, 0, ZERO_BYTES32, { from: tokenHolder });
          ...
          it('emits a TransferByPartition event', async function () {
            const { logs } = await this.token.transferByPartition(partition1, recipient, transferAmount, ZERO_BYTES32, { from: tokenHolder });
  
            assert.equal(logs.length, 2);
  
            assertTransferEvent(logs, partition1, tokenHolder, tokenHolder, recipient, transferAmount, ZERO_BYTES32, null);
          });
        });
        describe('when the transfer amount is equal to 0', function () {
          it('reverts', async function () {
            await shouldFail.reverting(this.token.transferByPartition(partition2, recipient, 0, ZERO_BYTES32, { from: tokenHolder }));
          });
        });
      ...
      describe('when the sender does not have enough balance for this partition', function () {
        it('reverts', async function () {
          await shouldFail.reverting(this.token.transferByPartition(partition2, recipient, transferAmount, ZERO_BYTES32, { from: tokenHolder }));
        });
      });
  });
  ...
  describe('operatorTransferByPartition', function () {
    ...
    describe('when the sender is approved for this partition', function () {
      describe('when approved amount is sufficient', function () {
        it('transfers the requested amount', async function () {
          ...
          await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, ZERO_BYTE, ZERO_BYTES32, { from: operator });
      ...
      describe('when approved amount is not sufficient', function () {
        it('reverts', async function () {
          ...
          await this.token.approveByPartition(partition1, operator, approvedAmount, { from: tokenHolder });
          assert.equal(await this.token.allowanceByPartition(partition1, tokenHolder, operator), approvedAmount);
          await shouldFail.reverting(this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, ZERO_BYTE, ZERO_BYTES32, { from: operator }));
        });
      });
    });
    describe('when the sender is an operator for this partition', function () {
      describe('when the sender has enough balance for this partition', function () {
        describe('when partition does not change', function () {
          ...
          it('transfers the requested amount with attached data (without changePartition flag)', async function () {
            ...
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, doNotChangePartition, ZERO_BYTES32, { from: operator });
          ...
          it('emits a TransferByPartition event', async function () {
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            const { logs } = await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, ZERO_BYTE, ZERO_BYTES32, { from: operator });

            assert.equal(logs.length, 2);

            assertTransferEvent(logs, partition1, operator, tokenHolder, recipient, transferAmount, null, ZERO_BYTES32);
          });
        });
        describe('when partition changes', function () {
          it('transfers the requested amount', async function () {
            ...
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, changeToPartition2, ZERO_BYTES32, { from: operator });
          ...
          it('converts the requested amount', async function () {
            ...
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            await this.token.operatorTransferByPartition(partition1, tokenHolder, tokenHolder, transferAmount, changeToPartition2, ZERO_BYTES32, { from: operator });
          ...
          it('emits a changedPartition event', async function () {
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            const { logs } = await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, changeToPartition2, ZERO_BYTES32, { from: operator });

            assert.equal(logs.length, 3);

            assertTransferEvent([logs[0], logs[1]], partition1, operator, tokenHolder, recipient, transferAmount, changeToPartition2, ZERO_BYTES32);
      ...
      describe('when the sender does not have enough balance for this partition', function () {
        it('reverts', async function () {
          await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
          await shouldFail.reverting(this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, issuanceAmount + 1, ZERO_BYTE, ZERO_BYTES32, { from: operator }));
        });
      });
    });
  ...
  describe('revokeOperatorByPartition', function () {
    ...
    describe('isOperatorForPartition', function () {
        ...
        it('when operator is tokenHolder', async function () {
          assert.isTrue(await this.token.isOperatorForPartition(partition1, tokenHolder, tokenHolder));
        });
        it('when operator is authorized by tokenHolder', async function () {
          await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
          assert.isTrue(await this.token.isOperatorForPartition(partition1, operator, tokenHolder));
        ...
        it('when is a controller and token is controllable', async function () {
          assert.isTrue(await this.token.isOperatorForPartition(partition1, controller, tokenHolder));
        });
        it('when is a controller and token is not controllable', async function () {
            await this.token.renounceControl({ from: owner});
            assert.isTrue(!(await this.token.isOperatorForPartition(partition1, controller, tokenHolder)));
    ...
    describe('when sender is the issuer', function () {
        ...
        describe('when the amount is a multiple of the granularity', function () {
            describe('when the recipient is not the zero address', function () {
                it('issues the requested amount', async function () {
                    await this.token.issue(tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner });
                  ...
                  it('emits a issuedByPartition event', async function () {
                    const { logs } = await this.token.issue(tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner });
            ...
            describe('when the recipient is not the zero address', function () {
                it('issues the requested amount', async function () {
                    await shouldFail.reverting(this.token.issue(ZERO_ADDRESS, issuanceAmount, ZERO_BYTES32, { from: owner }));
                  });
            });
        });
        describe('when the amount is not a multiple of the granularity', function () {
            it('issues the requested amount', async function () {
                this.token = await ERC1400.new('ERC1400Token', 'DAU', 2, [controller], partitions);
                await shouldFail.reverting(this.token.issue(tokenHolder, 1, ZERO_BYTES32, { from: owner }));
              });
        });
      });
      describe('when token is not issuable', function () {
        it('reverts', async function () {
          ...
          assert.isTrue(!(await this.token.isIssuable()));
          await shouldFail.reverting(this.token.issue(tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner }));
        });
      });
    });
    describe('when sender is not the issuer', function () {
      it('reverts', async function () {
        await shouldFail.reverting(this.token.issue(tokenHolder, issuanceAmount, ZERO_BYTES32, { from: unknown }));
      });
    });
  });
  ...
  describe('issueByPartition', function () {
    ...
    describe('when sender is the issuer', function () {
      describe('when token is issuable', function () {
        ...
        it('emits a issuedByPartition event', async function () {
          const { logs } = await this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner });
      ...
      describe('when token is not issuable', function () {
        it('reverts', async function () {
          ...
          await this.token.renounceIssuance({ from: owner });
          assert.isTrue(!(await this.token.isIssuable()));
          await shouldFail.reverting(this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner }));
        });
      });
    });
    describe('when sender is not the issuer', function () {
      it('reverts', async function () {
        await shouldFail.reverting(this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, ZERO_BYTES32, { from: unknown }));
      });
    });
  });
        ...
        describe('when the amount is a multiple of the granularity', function () {
            describe('when the sender has enough balance for those default partitions', function () {
                it('redeeems the requested amount', async function () {
                  ...
                  await this.token.redeem(2.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder });
        
                  await assertBalances(this.token, tokenHolder, partitions, [0, 0.5 * issuanceAmount, 0]);
                });
                it('emits a redeemedByPartition events', async function () {
                  await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
                  const { logs } = await this.token.redeem(2.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder });
                  
                  assert.equal(logs.length, 3 * partitions.length);
        
                  assertBurnEvent([logs[0], logs[1], logs[2]], partition3, tokenHolder, tokenHolder, issuanceAmount, ZERO_BYTES32, null);
                  assertBurnEvent([logs[3], logs[4], logs[5]], partition1, tokenHolder, tokenHolder, issuanceAmount, ZERO_BYTES32, null);
                  assertBurnEvent([logs[6], logs[7], logs[8]], partition2, tokenHolder, tokenHolder, 0.5 * issuanceAmount, ZERO_BYTES32, null);
                });
              });
              describe('when the sender does not have enough balance for those default partitions', function () {
                it('reverts', async function () {
                  await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
                  await shouldFail.reverting(this.token.redeem(3.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder }));
                });
              });
        });
        describe('when the amount is not a multiple of the granularity', function () {
            it('reverts', async function () {
                ...
                await shouldFail.reverting(this.token.redeem(3, ZERO_BYTES32, { from: tokenHolder }));
              });
        });
    });
    describe('when defaultPartitions have not been defined', function () {
      it('reverts', async function () {
        await this.token.setDefaultPartitions([], { from: owner });
        await shouldFail.reverting(this.token.redeem(2.5 * issuanceAmount, ZERO_BYTES32, { from: tokenHolder }));
      });
    });
  });
            ...
            describe('when the amount is a multiple of the granularity', function () {
                describe('when the redeemer is not the zero address', function () {
                    it('redeems the requested amount', async function () {
                        ...
                        await this.token.redeemFrom(tokenHolder, 2.5 * issuanceAmount, ZERO_BYTES32, { from: operator });
            
                        await assertBalances(this.token, tokenHolder, partitions, [0, 0.5 * issuanceAmount, 0]);
                      });
                      it('emits redeemedByPartition events', async function () {
                        await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
                        const { logs } = await this.token.redeemFrom(tokenHolder, 2.5 * issuanceAmount, ZERO_BYTES32, { from: operator });
            
                        assert.equal(logs.length, 3 * partitions.length);
            
                        assertBurnEvent([logs[0], logs[1], logs[2]], partition3, operator, tokenHolder, issuanceAmount, ZERO_BYTES32, null);
                        assertBurnEvent([logs[3], logs[4], logs[5]], partition1, operator, tokenHolder, issuanceAmount, ZERO_BYTES32, null);
                        assertBurnEvent([logs[6], logs[7], logs[8]], partition2, operator, tokenHolder, 0.5 * issuanceAmount, ZERO_BYTES32, null);
                      });
                });
                describe('when the redeemer is the zero address', function () {
                    it('reverts', async function () {
                        ...
                        await shouldFail.reverting(this.token.redeemFrom(ZERO_ADDRESS, 2.5 * issuanceAmount, ZERO_BYTES32, { from: operator }));
                      });
                });           
            });
            describe('when the amount is not a multiple of the granularity', function () {
                it('reverts', async function () {
                    ...
                    await shouldFail.reverting(this.token.redeemFrom(tokenHolder, 3, ZERO_BYTES32, { from: operator }));
                  });
            });
        });
        describe('when the sender does not have enough balance for those default partitions', function () {
          it('reverts', async function () {
            await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
            await shouldFail.reverting(this.token.redeemFrom(tokenHolder, 3.5 * issuanceAmount, ZERO_BYTES32, { from: operator }));
          });
        });
      });
      describe('when defaultPartitions have not been defined', function () {
        it('reverts', async function () {
          await this.token.setDefaultPartitions([], { from: owner });
          await shouldFail.reverting(this.token.redeemFrom(tokenHolder, 2.5 * issuanceAmount, ZERO_BYTES32, { from: operator }));
        });
      });
    });
    describe('when the operator is not approved', function () {
  ...
  describe('redeemByPartition', function () {
    ...
    describe('when the redeemer has enough balance for this partition', function () {
      it('redeems the requested amount', async function () {
        await this.token.redeemByPartition(partition1, redeemAmount, ZERO_BYTES32, { from: tokenHolder });
      ...
      it('emits a redeemedByPartition event', async function () {
        const { logs } = await this.token.redeemByPartition(partition1, redeemAmount, ZERO_BYTES32, { from: tokenHolder });

        assert.equal(logs.length, 3);

        assertBurnEvent(logs, partition1, tokenHolder, tokenHolder, redeemAmount, ZERO_BYTES32, null);
      });
    });
    describe('when the redeemer does not have enough balance for this partition', function () {
      it('reverts', async function () {
        await shouldFail.reverting(this.token.redeemByPartition(partition2, redeemAmount, ZERO_BYTES32, { from: tokenHolder }));
      });
    });
    describe('special case (_removeTokenFromPartition shall revert)', function () {
      it('reverts', async function () {
        await this.token.issueByPartition(partition2, owner, issuanceAmount, ZERO_BYTES32, { from: owner });
        await shouldFail.reverting(this.token.redeemByPartition(partition2, 0, ZERO_BYTES32, { from: tokenHolder }));
      });
    });
  });
  ...
  describe('operatorRedeemByPartition', function () {
    ...
    describe('when the sender is an operator for this partition', function () {
      describe('when the redeemer has enough balance for this partition', function () {
        it('redeems the requested amount', async function () {
          await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
          await this.token.operatorRedeemByPartition(partition1, tokenHolder, redeemAmount, ZERO_BYTES32, { from: operator });
        ...
        it('emits a redeemedByPartition event', async function () {
          await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
          const { logs } = await this.token.operatorRedeemByPartition(partition1, tokenHolder, redeemAmount, ZERO_BYTES32, { from: operator });

          assert.equal(logs.length, 3);

          assertBurnEvent(logs, partition1, operator, tokenHolder, redeemAmount, null, ZERO_BYTES32);
        });
      });
      describe('when the redeemer does not have enough balance for this partition', function () {
        it('reverts', async function () {
          it('redeems the requested amount', async function () {
            ...
            await shouldFail.reverting(this.token.operatorRedeemByPartition(partition1, tokenHolder, issuanceAmount + 1, ZERO_BYTES32, { from: operator }));
          });
        });
      });
    });
    ...
    describe('when the sender is not an operator', function () {
      it('reverts', async function () {
        await shouldFail.reverting(this.token.operatorRedeemByPartition(partition1, tokenHolder, redeemAmount, ZERO_BYTES32, { from: operator }));
      });
    });
  });
  ...
  describe('parameters', function () {
    ...
    describe('totalPartitions', function () {
        it('returns the list of partitions', async function () {
            ...
            await this.token.issueByPartition(partition2, tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner });
            totalPartitions = await this.token.totalPartitions();
            assert.equal(totalPartitions.length, 2);
            assert.equal(totalPartitions[0], partition1);
            assert.equal(totalPartitions[1], partition2);

            await this.token.issueByPartition(partition3, tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner });
            totalPartitions = await this.token.totalPartitions();
  ...
  describe('migrate', function () {
    ...
    beforeEach(async function () {
      this.token = await ERC1400.new('ERC1400Token', 'DAU20', 1, [controller], partitions);
      this.migratedToken = await ERC1400.new('ERC1400Token', 'DAU20', 1, [controller], partitions);
      await this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, ZERO_BYTES32, { from: owner });
    });
    describe('when the sender is the contract owner', function () {
    ...
    describe('when the contract is migrated definitely', function () {
      it('can not transfer tokens', async function () {
        ...
        await shouldFail.reverting(this.token.transferByPartition(partition1, recipient, transferAmount, ZERO_BYTES32, { from: tokenHolder }));
      });
    });
