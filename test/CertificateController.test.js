...
contract('ERC1400 with CertificateController', ([owner, operator, controller, tokenHolder, recipient, unknown]) => {
    ...
    describe('when the sender is the contract owner', function () {
      describe('when the new certificate signer address is valid', function () {
        ...
        it('sets the operator as certificate signer', async function () {
          ...
          await this.token.setCertificateSigner(operator, true, { from: owner });
          assert(await this.token.certificateSigners(operator));
          await this.token.setCertificateSigner(operator, false, { from: owner });
          assert(!(await this.token.certificateSigners(operator)));
        });
      });
      describe('when the certificate signer address is not valid', function () {
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(
            this.token.setCertificateSigner(ZERO_ADDRESS, true, { from: owner }),
            '' // Action Blocked - Not a valid address
          );
        });
        it('reverts', async function () {
          await shouldFail.reverting.withMessage(
            this.token.setCertificateSigner(ZERO_ADDRESS, false, { from: owner }),
            '' // Action Blocked - Not a valid address
          );
        });
      });
  ...
  describe('transferByPartition', function () {
    ...
    describe('when thecertifiacte is not valid', function () {
      it('reverts', async function () {
        await shouldFail.reverting(this.token.transferByPartition(partition1, recipient, transferAmount, INVALID_CERTIFICATE, { from: tokenHolder }));
      });
    });
  });
